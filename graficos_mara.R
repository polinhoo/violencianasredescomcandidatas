...
mara <- read_excel("marareduanalisada.xlsx")
readr::guess_encoding("marareduanalisada.xlsx")
mara$ofensa <- str_replace_all(mara$ofensa, "\"", "")
mara$ofensa <- str_replace_all(mara$ofensa, ",", " ")
mara$ofensa <- str_replace_all(mara$ofensa, "#", " ")
mara$ofensa <- str_replace_all(mara$ofensa, "\r", " ")
mara$ofensa <- str_replace_all(mara$ofensa, "\n", " ")
mara$ofensa <- str_replace_all(mara$ofensa, "\t", " ")
mara$ofensa <- str_replace_all(mara$ofensa, "@", " ")
mara$ofensa <- str_replace_all(mara$ofensa, "[[:punct:]]", " ")
mara$ofensa <- str_replace_all(mara$ofensa, "Ã ", "á")
mara$ofensa <- str_replace_all(mara$ofensa, "áª", "ê")
mara$ofensa <- str_replace_all(mara$ofensa, "á£", "ã")
mara$ofensa <- str_replace_all(mara$ofensa, "á©", "é")
mara$ofensa <- str_replace_all(mara$ofensa, "Ã©", "é")
mara$ofensa <- str_replace_all(mara$ofensa, "á", "ó")
mara$ofensa <- str_replace_all(mara$ofensa, "Ãª", "ê")
mara$ofensa <- str_replace_all(mara$ofensa, "Ã£", "ã")
mara$ofensa <- str_replace_all(mara$ofensa, "Ãµ", "õ")
mara$ofensa <- str_replace_all(mara$ofensa, "Ã­", "í")
mara$ofensa <- str_replace_all(mara$ofensa, "Ã³", "ó")
mara$ofensa <- str_replace_all(mara$ofensa, "Ã£", "ã")
mara$ofensa <- str_replace_all(mara$ofensa, "ðŸ¤®", " ")
mara$ofensa <- str_replace_all(mara$ofensa, "£ðŸ", " ")
mara$ofensa <- str_replace_all(mara$ofensa, "ðÿ", " ")
mara$ofensa <- str_replace_all(mara$ofensa, "£", " ")
mara$ofensa <- str_replace_all(mara$ofensa, "corrupáão", "corrupção")
mara$ofensa <- str_replace_all(mara$ofensa, "esperanáa", "esperança")
mara$ofensa <- str_replace_all(mara$ofensa, "simonetebetbr", " ")
#exportando em csv
write.table(mara, "marareduanalisada.csv", row.names = F, sep = "|")
mara2 <- read.csv("marareduanalisada.csv", sep = "|") 
##ofensa
table(mara2$contra.ela)
mara2$contra.ela[which(mara2$contra.ela=="\nnao")] <- "não"
mara2$contra.ela[which(mara2$contra.ela=="\nsim")] <- "sim"
mara2$contra.ela[which(mara2$contra.ela=="nao dá pra saber")] <- "não"
mara2$contra.ela[which(mara2$contra.ela=="não dá pra saber")] <- "não"
mara2$contra.ela[which(mara2$contra.ela=="não")] <- "não"
mara2$contra.ela[which(mara2$contra.ela=="nao")] <- "não"

ofensa <- grep("sim", mara2$contra.ela)
ofensa <- mara2[ofensa,2]

neutro <- grep("não", mara2$contra.ela)
neutro <- mara2[neutro,2]
...
ofensa <- str_replace_all (ofensa, "maragabrilli ", " ")
...
neutro <- str_replace_all (neutro, "maragabrilli ", " ")
...
write.csv(freq_x,"palavras_contra_mara.csv", row.names = F)
write.csv(freq_z,"palavras_favor_mara.csv", row.names = F)
...
mara2$contra.ela[which(mara2$contra.ela=="sim")] <- "ofensivo contra ela"
mara2$contra.ela[which(mara2$contra.ela=="não")] <- "não ofensivo a ela"
library(ggplot2)

ggplot(mara2, aes(contra.ela, fill=contra.ela))+
  geom_bar(show.legend = F)+
  labs(x="", y="Quantidade", fill="", 
       title = "Como foram os tweets direcionados à Mara Gabrilli", 
       subtitle = "155 ofensivos a ela e 188 não ofensivos a ela")+
