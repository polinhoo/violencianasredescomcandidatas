...
janaina <- read.csv("janainaredu.csv")
##ofensa
ofensa <- grep("SIM", janaina$ofensivo)
ofensa <- janaina[ofensa,2]

neutro <- grep("NÃO", janaina$ofensivo)
neutro <- janaina[neutro,2]
...
ofensa <- str_replace_all (ofensa, "janainadobrasil ", " ")
...
neutro <- str_replace_all (neutro, "janainadobrasil ", " ")
...
write.csv(freq_x,"palavras_contra_janaina.csv", row.names = F)
write.csv(freq_z,"palavras_favor_janaina.csv", row.names = F)
...
##uma tabela contando
table(janaina$ofensivo)

##um gráfico de barras
janaina$ofensivo[which(janaina$ofensivo=="SIM")] <- "ofensivo contra ela"
janaina$ofensivo[which(janaina$ofensivo=="NÃO")] <- "não ofensivo a ela"
ggplot(janaina, aes(ofensivo, fill=ofensivo))+
  geom_bar(show.legend = F)+
  labs(x="", y="Quantidade", fill="", 
       title = "Como foram os tweets direcionados à Janaína Paschoal", 
       subtitle = "199 ofensivos a ela e 144 não ofensivos a ela")+
